<div align="center">

# Hi, I'm Mina Adel 👋

### Mechanical Maintenance & Design Engineer  
### PLC & Industrial Automation | Embedded Systems | Embedded Linux | PCB Design | R&D 
### TPM & Continuous Improvement | CNC | Robotics | Simulation | Electric & Electronics
<img src="https://komarev.com/ghpvc/?username=MADSA-create&style=flat&color=blue" />

![PLC](https://img.shields.io/badge/PLC-Automation-blue?style=flat)
![Embedded](https://img.shields.io/badge/Embedded-Systems-darkgreen?style=flat)
![PCB](https://img.shields.io/badge/PCB-Design-orange?style=flat)
![SolidWorks](https://img.shields.io/badge/SolidWorks-Design-red?style=flat)
![ESP32](https://img.shields.io/badge/ESP32-IoT-black?style=flat)

</div>

---

# 🚀 About Me

Mechanical & Automation Engineer with experience in:

- Mechanical Maintenance
- Industrial Automation
- PLC Systems
- Mechanical Design
- Embedded Systems
- PCB Design
- TPM & Continuous Improvement

Passionate about improving machine performance, increasing production efficiency, and developing smart industrial solutions.

Experienced in troubleshooting industrial machines, preventive maintenance, and integrating automation with mechanical systems to build reliable and efficient industrial solutions.

---

# ⚡ Core Skills
- Robotics
- ROS
- ROS2
- Linux OS
## 🏭 Industrial Automation
- PLC Programming
- Industrial Automation
- Sensors & Actuators
- Motor Control Systems
- Control Panels
- Industrial Troubleshooting

---

## ⚙️ Mechanical Engineering
- Mechanical Maintenance
- Mechanical Design
- Machine Troubleshooting
- Preventive Maintenance
- TPM Implementation
- Continuous Improvement

---

## 🔌 Embedded Systems
- Arduino
- ESP32
- STM32 Basics
- IoT Concepts
- Smart Monitoring Systems

---

## 🔧 PCB Design
- PCB Layout Design
- Circuit Design
- Electronic Troubleshooting
- Proteus / KiCad / Altium Basics

---

## 🛠 Software & Tools
- SolidWorks
- AutoCAD
- TIA Portal
- Proteus
- Microsoft Office

---

# 🔥 Featured Projects

## 🏭 Automatic Packaging Machine Control System

### Overview
Designed and improved an industrial packaging machine control system using PLC technology to enhance machine performance and operational stability.

### Key Features
- Automated machine sequence control
- Sensor integration
- Motor synchronization
- Fault detection system
- Reduced downtime

### Technologies Used
- PLC
- Sensors
- Motor Control
- Industrial Automation

### Results
✅ Improved machine reliability  
✅ Reduced operational faults  
✅ Increased production efficiency  

---

## ⚙️ Mechanical Design for Packaging Machine

### Overview
Designed mechanical components for industrial packaging systems to improve machine stability and production efficiency.

### Key Features
- 3D Mechanical Design
- Motion Optimization
- Structural Improvement
- Machine Modification

### Tools Used
- SolidWorks
- AutoCAD

### Results
✅ Improved machine performance  
✅ Reduced vibration  
✅ Enhanced operational reliability  

---

## 🌡 Smart Monitoring System using ESP32

### Overview
Developed a smart monitoring system for industrial applications using embedded systems technology.

### Key Features
- Temperature monitoring
- Wireless data transmission
- Real-time monitoring
- Alarm notification system

### Technologies Used
- ESP32
- Embedded Systems
- Sensors
- IoT Concepts

### Results
✅ Faster fault detection  
✅ Improved monitoring process  
✅ Increased system reliability  

---

## 🔌 Industrial PCB Design Project

### Overview
Designed custom PCB boards for industrial control applications.

### Key Features
- Compact PCB layout
- Reliable industrial connections
- Optimized circuit routing
- Improved system stability

### Tools Used
- Proteus
- KiCad
- PCB Design Tools

### Results
✅ Reduced wiring complexity  
✅ Improved reliability  
✅ Easier maintenance  

---

## 📈 TPM & Continuous Improvement Project

### Overview
Participated in TPM and continuous improvement activities focused on reducing downtime and increasing machine availability.

### Key Features
- Root cause analysis
- Preventive maintenance planning
- OEE improvement
- Downtime reduction

### Results
✅ Increased machine availability  
✅ Improved operational efficiency  
✅ Reduced maintenance losses  

---

# 📚 Certifications & Training

- PLC Programming
- Industrial Automation
- Mechanical Maintenance
- Embedded Systems
- TPM & Continuous Improvement

---

# 🎯 Career Objective

Seeking opportunities in industrial automation, maintenance engineering, mechanical design, and R&D where I can contribute to operational excellence, smart industrial solutions, and continuous improvement.

---

# 🌐 Connect With Me

<div align="left">

<a href="https://www.linkedin.com/in/mina-adel-539916208" target="_blank">
  <img src="https://skillicons.dev/icons?i=linkedin" width="55" />
</a>

<a href="https://github.com/MADSA-create/Mina-Adel" target="_blank">
  <img src="https://skillicons.dev/icons?i=github" width="55" />
</a>

</div>

<br>

<div align="left">

📍 Egypt 🇪🇬

</div>
